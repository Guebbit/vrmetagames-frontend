      ...
      <div class="container flex justify-between relative items-center md:flex-row flex-col-reverse">
        ...
        <client-only>
          <back-to-top class="md:absolute-center position-absolute cursor-pointer z-999 left-3/4 md:left-1/2 mb-15 sm:mb-auto" bottom="auto" right="auto">
            <img src="public/images/icon/scroll-top.webp" alt="">
          </back-to-top>
        </client-only>
        ...
        <ul class="flex">
          <li v-for="(link, index) in links" :key="index" class="text-white content-last-child-before before:ml-3 before:mr-3 before:inline-block before:content-['||'] first:before:hidden">
            <router-link :to="`${link.pageLink}`" class="text-gray-500 text-sm uppercase font-medium transition-all hover:text-primary">{{link.linkName}}</router-link>
          </li>
        </ul>
...
export default defineComponent({
  components: {
    FooterInfoWidget: () => import('@/components/Footer/FooterInfoWidget.vue'),
    FooterContactInfoWidget: () => import('@/components/Footer/FooterContactInfoWidget.vue'),
    FooterWinnerWidget: () => import('@/components/Footer/FooterWinnerWidget.vue'),
    FooterMenuList: () => import('@/components/Footer/FooterMenuList.vue')
  },
  data() {
    return {
      links: [
