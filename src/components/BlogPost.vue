      ...
      <div v-for="(blog, index) in blogData.slice(0, 4)" :key="index">
        <div class="flex sm:items-center flex-col sm:flex-row">
          <div class="lg:w-40 xl:w-52 2xl:w-52 md:w-44 sm:w-37 w-32 mb-7 sm:mb-0 flex-shrink-0">
            <router-link :to="`/blog/${blog.slug}`"><img :src="blog.imgSrc" :alt="blog.alt"></router-link>
          </div>
          <div class="xl:ml-9 lg:ml-6 md:ml-9 ml-3 flex flex-col flex-1">
            ...
            <router-link :to="`/blog/${blog.slug}`" class="uppercase font-exo relative pl-12 text-sm text-white transition-all hover:text-primary  before:content-[''] before:absolute before:h-0.5 before:w-7.5 before:bg-primary before:left-0 before:top-1/2 before:transform before:-translate-y-1/2">Read More</router-link>
          </div>
        </div>
      </div>
    </div>
